...
function scoreboard(getInningScoreCB, inningCB, inningsLeft) {
  let homeScore = 0;
  let awayScore = 0;
  const inningScore = [];
  for (let i = 0; i < inningsLeft; i++) {
    let inningObject = getInningScore(inningCB);
    inningScore.push(
      `Inning ${i + 1}: Away ${inningObject.Away} - Home ${inningObject.Home}`
    );
    homeScore = homeScore + inningObject.Home;
    awayScore = awayScore + inningObject.Away;
  }
  if (homeScore === awayScore) {
    ...
    return [inningScore, `Final Score: Away ${awayScore} - Home ${homeScore}`];
  }
}
